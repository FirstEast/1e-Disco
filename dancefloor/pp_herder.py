...
# PPHerder is the discovery protocol listener for PixelPusher devices.

class PPHerder(object):
  ...
  def __init__(self, existing_socket=None, io_loop=None, timeout=3):
    ...
    logging.info(u"Yo dawg, I heard you like derps...")
  ...
  def remove_callback(self, callback):
    ...
    self.found_callbacks.discard(callback)
    self.lost_callbacks.discard(callback)
  ...
  def _handle_socket_events(self, fd, events):
    ...
    try:
      ...
      if events & self.io_loop.ERROR:
        logging.error(u"Process Interrupted.")
        self.close()
    except Exception:
      self.close()
      raise
...
def main():
  def found_fxn(x):
    print u"Found a new device! %s  -  %s" % (x.packet.header.mac_str(), x.packet.header.ip_str())
  def lost_fxn(x):
    print u"We seem to have lost the device: %s" % x.packet.header.ip_str()

  logging.basicConfig(level=logging.INFO)
  logging.info(u"Starting bare device discovery demo.")

  shepherd = PPHerder()
  shepherd.add_callbacks(found=found_fxn, lost=lost_fxn)
