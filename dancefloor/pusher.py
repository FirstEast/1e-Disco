...
class RemotePixelPusher(object):
  u"""This lets you schedule periodic pixelpusher updates."""
  ...
  def update_period(self):
    u"""TODO: normalize time unit?"""
    return self.packet.p.pixelpusher.update_period

  def update_discovery(self, new_packet):
    u"""Returns the old buffer."""
    logging.info(u"DiscoveryPacket[%s]- power[%d] update_period[%d] delta_sequence[%d] ip[%s]",
  ...
  def pp_addr(self):
    return (u"%d.%d.%d.%d" % tuple(self.packet.header.ip_address), PIXEL_PORT)

  def _push_pixels(self):
    u"""This sends a pixel packet and then reschedules itself."""
    next_packet = self.packet_gen.next()
    message = struct.pack(u"i", self.seq_number) + bytearray(next_packet)
  ...
  def start_updates_with_source(self, fxn):
    u"""We will call fxn periodically with strip id as arg."""
