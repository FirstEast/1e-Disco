do ->
  class com.firsteast.PatternSelector extends Backbone.View
    ...
    render: =>
      ...
      partyWorthySaveModels = _.sortBy(@savedPatternList.where({partyWorthy: true}), (x) -> return x.get('name'))
      nonPartyWorthySaveModels = _.sortBy(@savedPatternList.where({partyWorthy: false}), (x) -> return x.get('name'))
